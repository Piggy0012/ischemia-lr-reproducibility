...
def build(lang):
    raw=(ROOT/f'repro_manuscript_{lang}_source.md').read_text(encoding='utf-8')
    assert '<!-- PENDING' not in raw and '本稿不宣称' not in raw
    order=[]
    def cite(m):
        key=m.group(1);assert key in REFS and REFS[key]['metadata_status']=='crossref_verified',key
    ...
    (OUT/f'manuscript_{lang}.md').write_text(md,encoding='utf-8')
    (OUT/f'references_{lang}.json').write_text(json.dumps([REFS[k] for k in order],ensure_ascii=False,indent=2),encoding='utf-8')
    doc=Document()
    ...
    while i<len(lines):
        ...
        if line.startswith('# '):inline(doc.add_paragraph(style='Title'),line[2:].replace('ligand–receptor','ligand\u2060–\u2060receptor'));continue
        if line.startswith('## '):
    ...
    path=OUT/f'manuscript_{lang}.docx';doc.save(path)
    return {'language':lang,'docx':str(path),'references':len(order),'figures':len(figures),'tables':len(doc.tables)}

if __name__=='__main__':
    reports=[build(lang) for lang in ['zh','en']]
