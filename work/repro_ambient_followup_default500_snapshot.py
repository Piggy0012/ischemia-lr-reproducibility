...
def process(acc,sample):
    ...
    if done.exists(): print('AMBIENT AGG EXISTS',sample,flush=True);return
    source=ROOT/'repro_decontx'/acc/sample
    assert (source/'audit.json').exists(),f'DecontX not complete: {sample}'
