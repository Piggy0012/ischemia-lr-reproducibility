...
import os,sys,json,gc,hashlib,shutil
...
def process(acc,sample):
    ...
    if done.exists():
        ...
        historical=OUT/'historical_default500_aggregates'/acc/sample
        historical.mkdir(parents=True,exist_ok=True)
        for old in folder.glob(sample+'_*'):
            if old.is_file():shutil.copy2(old,historical/old.name)
