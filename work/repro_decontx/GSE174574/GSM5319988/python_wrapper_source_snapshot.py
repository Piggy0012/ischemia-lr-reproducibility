    ...
    if not (dest/'r_audit.json').exists():
        if not (dest/'input_audit.json').exists():
            ...
            meta={'dataset':acc,'sample':sample,'n_genes':x.shape[1],'n_cells':x.shape[0],
                  ...
                  'script_sha256':{name:digest(ROOT/name) for name in ['repro_decontx.py','repro_decontx_run.R','repro_decontx_plan.md']},
        ...
        command=[str(RSCRIPT),'--vanilla',str(ROOT/'repro_decontx_run.R'),str(RLIBRARY),str(dest)]
