...
We added a controlled-resource analysis with the native CellChat R package (2.2.0.9001) using the same 11 raw-count libraries and frozen primary cell labels. The catalogue contained 1,548 shared, gene-covered interaction definitions with native complexes and cofactors. Rankings were calculated on 1,222 source–target–interaction entries with complete archived LIANA scores in every library. We compared native CellChat inferred strength, an analyst-derived strength percentile, expression coavailability and LIANA RRA recomputed once per distinct score column on this fixed network. These additional cohort contrasts were descriptive; the original permutation P values were not applied to them. [@Jin2025CellChat][@Jin2021CellChat]
