...
max_iter <- as.integer(Sys.getenv("DECONTX_MAX_ITER","500"))
stopifnot(max_iter %in% c(500L,2000L))
...
fit <- decontX(x, z=cells$whole_brain_broad, background=NULL, batch=NULL,
              ...
              maxIter=max_iter, convergence=0.001, iterLogLik=10,
              verbose=TRUE)
...
# The immutable native sparse slots are converted to lossless Matrix Market
# by the single-threaded Python transport after this R process exits.
...
summary <- list(dataset=meta$dataset,sample=meta$sample,
                ...
                requested_parameters=list(z_source="whole_brain_broad",background=NULL,batch=NULL,
                  seed=20260911,delta=c(10,10),estimateDelta=TRUE,maxIter=max_iter,
